...
bool KeyManager::LoadFromFile() {
    try {
        ...
        if (!std::filesystem::exists(keysPath)) {
            SetDefaultKeys();
            SaveToFile();
            LogInfo("Created default key file: " + keysPath.string());
            return true;
        }

        std::ifstream file(keysPath);
        if (!file.is_open()) {
            LogError("Could not open key file: " + keysPath.string());
            return false;
        }
        ...
        m_keys.FakeKeyset.PublicExponent = {0, 1, 0, 1};

        m_keys.FakeKeyset.Coefficient = {
        ...
        m_keys.FakeKeyset.PrivateExponent = {
        ...
        m_keys.PkgDerivedKey3Keyset.Exponent1 = {
        ...
        m_keys.PkgDerivedKey3Keyset.Exponent2 = {
        ...
        m_keys.PkgDerivedKey3Keyset.PublicExponent = {0, 1, 0, 1};

        m_keys.PkgDerivedKey3Keyset.Coefficient = {
        ...
        m_keys.PkgDerivedKey3Keyset.Modulus = {
        ...
        m_keys.PkgDerivedKey3Keyset.Prime1 = {
        ...
        m_keys.PkgDerivedKey3Keyset.Prime2 = {
        ...
        m_keys.PkgDerivedKey3Keyset.PrivateExponent = {
        ...
        LogInfo("Successfully loaded keys from: " + keysPath.string());
        return true;

    } catch (const std::exception& e) {
        LogError("Error loading keys, using defaults: " + std::string(e.what()));
        SetDefaultKeys();
        return false;
    }
}
...
bool KeyManager::SaveToFile() {
    try {
        ...
        if (!file.is_open()) {
            LogError("Could not open key file for writing: " + keysPath.string());
            return false;
        }
        ...
        if (file.fail()) {
            LogError("Failed to write keys to: " + keysPath.string());
            return false;
        }

        LogInfo("Successfully saved keys to: " + keysPath.string());
        return true;

    } catch (const std::exception& e) {
        LogError("Error saving keys: " + std::string(e.what()));
        return false;
    }
}
...
bool KeyManager::HasKeys() const {
    return !m_keys.TrophyKeySet.ReleaseTrophyKey.empty() || !m_keys.FakeKeyset.Modulus.empty() ||
           !m_keys.DebugRifKeyset.Modulus.empty() || !m_keys.PkgDerivedKey3Keyset.Modulus.empty();
}
